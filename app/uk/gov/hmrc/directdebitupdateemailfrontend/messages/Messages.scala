...
object Messages {
  ...
  object ServicePhase {
    ...
    def bannerText(link: String): Message = Message(
      english =
        s"""This is a new service – your <a class="govuk-link" href="$link">feedback</a> will help us to improve it.""",
      welsh =
        s"""Mae hwn yn wasanaeth newydd – bydd eich <a class="govuk-link" href="$link">adborth</a> yn ein helpu i’w wella."""
    )
  }
    ...
    def `We cannot contact you`(taxRegime: TaxRegime, bouncedEmail: Email): Message = {
      ...
      taxRegime match {
        case TaxRegime.Paye =>
          Message(
            english =
              s"We cannot contact you about your Employers’ PAYE Direct Debit using ${bouncedEmail.value.decryptedValue}.",
            welsh =
              s"Ni allwn gysylltu â chi am eich Debyd Uniongyrchol ar gyfer TWE y Cyflogwr gan ddefnyddio ${bouncedEmail.value.decryptedValue}."
          )
  ...
  object EmailConfirmed {
    ...
    def `We'll use ... to contact you about your Direct debit`(email: Email): Message = Message(
      ...
      welsh =
        s"Byddwn yn defnyddio <strong>${email.value.decryptedValue}</strong> i gysylltu â chi ynghylch eich Debyd Uniongyrchol."
    )
  ...
  object TooManyEmailAddresses {
    ...
    def `You have been locked out because you have tried to verify too many email addresses`(
      ...
      english =
        s"""You have been locked out because you have tried to verify too many email addresses. Please try again on <strong>$date at $time</strong>.""",
      welsh =
        s"""Rydych chi wedi cael eich cloi allan oherwydd eich bod wedi ceisio dilysu gormod o gyfeiriadau e-bost. Rhowch gynnig arall arni ar <strong>$date am $time</strong>."""
    )
  ...
  object PageUnavailable {
    ...
    def `Go to your tax account to check or change your Direct Debit email address.`(url: String): Message = Message(
      english =
        s"""<a class="govuk-link" href="$url">Go to your tax account</a> to check or change your Direct Debit email address.""",
      welsh =
        s"""<a class="govuk-link" href="$url">Ewch i’ch cyfrif treth</a> i wirio neu i newid eich cyfeiriad e-bost ar gyfer Debyd Uniongyrchol."""
    )
